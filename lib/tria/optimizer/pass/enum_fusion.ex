defmodule Tria.Optimizer.Pass.EnumFusion do
  ...
  defguard is_fusable(module, function, arity)
           when module == Enum or module == Stream or
                  (module == Map and function == :new) or
                  (module == :lists and function == :reverse and arity == 1)
  ...
  def run_while(ast, opts \\ []) do
    if opts[:return_depmap] do
      postwalk(ast, %Depmap{}, fn
        dot_call(module, function, args) = ast, depmap
        when is_fusable(module, function, length(args)) ->
  ...
  def unstream([{Stream, op, args}, {Enum, :to_list, []} | tail], state)
      when op in ~w[map filter]a do
  ...
  def join_consecutive(
        [{left_eos, left_op, [left]}, {right_eos, right_op, [right]} | tail],
        state
      )
      when is_eos(left_eos) and is_eos(right_eos) do
    if left_eos == Stream or is_fusable_fn(left) or is_fusable_fn(right) do
      case {left_op, right_op} do
        ...
        _ ->
          {left_eos, left_op, [left]} >>>
            join_consecutive([{right_eos, right_op, [right]} | tail], state)
      end
    else
      {left_eos, left_op, [left]} >>>
        join_consecutive([{right_eos, right_op, [right]} | tail], state)
  ...
  ### Joining with reduce
  def join_consecutive(
        [{eos, op, [func]}, {Enum, :reduce, [accumulator, reducer]} = step | tail],
        state
      ) do
    if eos == Stream or (is_pure(accumulator) and (is_fusable_fn(func) or is_fusable_fn(reducer))) do
      case op do
        ...
        :filter ->
          ...
          {body, state} =
            evaluate(tri(fn x, acc -> if func.(x), do: reducer.(x, acc), else: acc end), state)
        ...
        :flat_map ->
          ...
          {body, state} =
            evaluate(tri(fn x, acc -> Enum.reduce(func.(x), acc, reducer) end), state)
  ...
  def join_consecutive(
        [{_eos, :concat, []}, {Enum, :reduce, [accumulator, reducer]} | tail],
        state
      ) do
  ...
  def join_consecutive(
        [{Enum, :reduce, [[], reducer]} = reduce, {:lists, :reverse, []} = reverse | tail],
        state
      ) do
    case reducer do
  ...
  ### with Map.new
  def join_consecutive([{eos, :map, [left]} = first, {Map, :new, [right]} = second | tail], state)
      when is_eos(eos) do
  ...
  defp unchain(dot_call(eos, function, [subj | args]), acc)
       when is_fusable(eos, function, length(args) + 1) do
